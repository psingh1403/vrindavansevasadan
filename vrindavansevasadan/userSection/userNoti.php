   ...
   <body>
    ...
    <main>
        ...
        <div class="content">
            ...
            $sql = "SELECT `subject`, `reply` FROM `replymessage` WHERE user_id = $user_id ORDER BY reply_id DESC";
            $run = mysqli_query($con,$sql);
            if (mysqli_num_rows($run) >= 1){
            while ($row = mysqli_fetch_array($run)) {
             ?>
            <div class="container ml-5 mt-5">
                <h3><span>Q.</span> <?php echo $row['subject']; ?></h3>
                <p><span>Answer: </span> <?php echo $row['reply']; ?></p>
            </div>
        <?php } }else{
            echo "<h4 class='mt-5 p-5'>Nothing Found Please First ask your Query...</h4>";
        }
        ?>
        </div>
    </main>
