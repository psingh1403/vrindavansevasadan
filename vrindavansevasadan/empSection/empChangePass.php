...
<html class="no-js" lang="zxx">
   ...
   <body>
    ...
    <main>
        ...
        <div class="content"><h1>Change Your Password</h1>
            <div class="container mt-5">
                <div class="row justify-content-center">
                    <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-12 mx-auto form p-4">
                        <form action="#" method="post">
                            ...
                            <div class="float-right">
                                <button type="submit" name="empChangePass" class="btn btn-primary">Change</button>
                            </div>
                        </form>
                    </div>
                </div>
...
if (isset($_POST['empChangePass'])) {
    include '../admin/DataBaseConnection.php';
    $emp_id = $_SESSION['empid'];
    $emp_user = $_POST['username'];
    $emp_email = $_POST['email'];
    $emp_pass = $_POST['password'];
    $emp_new_pass = $_POST['ConfPassword'];

    $sql = "SELECT `emp_username`, `emp_email`, `emp_password` FROM `employee` WHERE `emp_username` = '$emp_user' AND `emp_email` = '$emp_email' AND `emp_password` = '$emp_pass'";

    $run = mysqli_query($con,$sql);
    if (mysqli_num_rows($run) < 1) {
        echo "<script>alert('Check your username and email...');</script>";
    }else{
        $quy = "UPDATE `employee` SET `emp_password` = '$emp_new_pass' WHERE `emp_id` = $emp_id";
        mysqli_query($con,$quy);
        echo "<script>alert('Your Password has been successfully updated thank you...');window.open('empProfile.php','_self');</script>";
    }
}
